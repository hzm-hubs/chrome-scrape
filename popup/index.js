      ...
      chrome.tabs.sendMessage(
        ...
        function (response) {
          ...
          if (chrome.runtime.lastError) {
            ...
            setTipsContent(
              `共获取到${response?.data.data.tableList?.length || 0}条数据`
            );
          }
          observeObj.loading = false;
        }
...
function handleCsv() {
  if (!readResult?.data.tableList.length) {
  ...
  let csv = "\n";
  // 表头
  if (readResult?.data.tableHeads.length) {
    csv += `\n${readResult.data.tableHeads.join(",")}\n`;
  }

  // 表内
  if (readResult?.data.tableList?.length) {
    readResult.data.tableList.forEach((l) => (csv += `${l.join(",")}\n`));
  }
